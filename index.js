...
var reload = document.querySelector("#newGame");
...
for(var i = 0; i < level.length; i++) {
    level[i].addEventListener("click", function() {
        ...
        if(this.textContent === "Peasy") {
            ...
            reset(12);
        } else if (this.textContent === "Spicy") {
            reset(25);
        }
    })
}
...
function reset(cells) {
    var cellsHorizontal = cells;
    var cellsVertical = cells;
    ...
    document.addEventListener('keydown', event => {
        ...
        if(event.keyCode === 73 || event.keyCode === 87) {
            Body.setVelocity(ball, { x, y: y - 5 });
        }
        if(event.keyCode === 76 || event.keyCode === 68) {
            Body.setVelocity(ball, { x: x + 5, y });
        }
        if(event.keyCode === 75 || event.keyCode === 83) {
            Body.setVelocity(ball, { x, y: y + 5 });
        }
        if(event.keyCode === 74 || event.keyCode === 65) {
            Body.setVelocity(ball, { x: x - 5, y});
        } 
    });
